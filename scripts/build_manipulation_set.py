...
``--swapper diffusion`` builds a third family: the target photograph is
regenerated by Stable Diffusion img2img at ``--strength``, so the whole face is
redrawn in latent space and no donor is involved. It needs the ``diffusion``
extra and a GPU to be practical.
...
Usage:
    ...
    python scripts/build_manipulation_set.py --swapper diffusion --strength 0.5
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--swapper", choices=("graphics", "inswapper", "diffusion"), default="graphics"
    )
    parser.add_argument(
    ...
    elif args.swapper == "diffusion":
        sys.path.insert(0, str(Path(__file__).resolve().parent))
        from evaluate_learned_watermark_attribution import Attacks
        from learned_watermark import pick_device

        attacks = Attacks(args.models, args.inswapper, pick_device())

        def swap(source: np.ndarray, target: np.ndarray) -> np.ndarray | None:
            return attacks.diffuse(target, args.strength, args.seed)

        method = (
            f"diffusion regeneration: Stable Diffusion v1.5 img2img of the target at "
            f"strength {args.strength}, 30 steps, no donor"
        )
        covers = ["diffusion regeneration of a real photograph"]
        does_not_cover = ["blending-based face swap", "GAN face swap", "full synthesis"]
        method_tag = f"sd15_img2img_{args.strength:g}"
    else:
